...
These assertion checks are deliberately implemented as initial blocks with $errors in order to be supported on \
the widest range of simulators. However, assertions need to be enabled.


.. literalinclude:: ../../../avl_axi/rtl/avl_axi.sv
    :language: verilog
...
AVL-AXI comes with a tools utility :func:`avl_axi._tools.get_verilog` to help integrate the library into your build environment.

This is exposed to the environment as the command line tool `avl_axi_get_verilog` which returns a list of all RTL files required \
for AVL_AXI.
...
Once connected the agent creates and internal :doc:`avl_axi.Interface </modules/avl_axi._interface>` object which is used to \
act as the physical interface to the AXI bus and share the parameters with the rest of the environment.

This internal interface in generated to serve 2 purposes:

    1. It abstracts the simulator specific behavior of the generate blocks. Some simulators flatten the generate blocks, while others do not.
