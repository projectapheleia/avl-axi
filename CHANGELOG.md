...
### Fixed
 - [PR #43](https://github.com/projectapheleia/avl-axi/pull/43) SequenceItem: `get_id()` read the AXI protocol ID (`awid`/`arid`) but `set_id()` was never overridden to match, so it silently wrote to `avl-core`'s unrelated `Transaction._id_` bookkeeping instead - a getter/setter imbalance. Overriding `set_id()` directly (as originally proposed) is unsafe: `avl-core`'s `Sequence.start_item()` calls `item.set_id()` with the sequence's own item index on every item sent, which would clobber any caller-specified `awid`/`arid`. Renamed the pair to `get_bus_id()`/`set_bus_id()` instead, matching the existing `get_addr()`/`set_addr()` convention, so it no longer collides with base sequencer bookkeeping.
