...
#
# Comb-ready reproducer (originally PR #41's axi5-comb-ready): one write + one
# read against a subordinate with a comb ready loop on every channel
# (rtl/example_hdl.sv). A watcher records whether each RTL-driven ready
# reached the wire. Gate: AW/AR/W ready must all be observed high.
#
#                 Verilator stock  Verilator fixed  Questa stock/fixed
#   awready/arready  collapsed        visible          visible
#   wready           collapsed        visible          visible
#   example.test     FAIL             PASS             PASS
#
# On stock Verilator the AW/AR/W comb pulses collapse to a zero-duration
# glitch: the driver's quiesce_control/quiesce_data clears the just-driven
# valid in the same RisingEdge callback that observed ready, and because
# ready is combinational on valid, the retraction collapses the pulse before
# any other coroutine scheduled on that same edge (e.g. the monitor) can
# observe it. Deferring the retracting write with `await NextTimeStep()`
# (see Driver.quiesce_control/quiesce_data/quiesce_response) holds the
# pulse for the rest of the timestep, fixing all three channels - unlike the
# original PR #41 fix, which patched only AW/AR reads and left W (drive_data)
# collapsing. Questa has no collapse, so no regression either way.
...
async def test(dut):
    ...
    # Gate: every comb-generated ready (AW/AR/W) must reach the wire.
    ...
    assert seen["wready"], "wready never observed high (W comb pulse collapsed)"
