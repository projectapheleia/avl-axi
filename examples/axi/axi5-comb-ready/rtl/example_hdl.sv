...
//
// Synthetic AXI subordinate touching every channel PR #41 patches:
//   AW/AR/W : RTL drives READY as a legal comb pulse (ready=f(valid)), asserted
//             after valid held 2 cycles. W is deliberately included -- PR #41
//             leaves its sample (drive_data) unpatched.
...
module example_hdl();
    ...
    // W ready (UNPATCHED-sample channel -- probes the PR #41 W gap)
    logic [3:0] w_wait;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)                                 w_wait <= '0;
