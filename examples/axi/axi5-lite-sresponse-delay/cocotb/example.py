...
async def test(dut):
    """
    Example AXI5-Lite Interface with subordinate response delays
    ...
    avl.Factory.set_variable("*.agent.swdrv.response_rate_limit", lambda: 0.5)
    avl.Factory.set_variable("*.agent.srdrv.response_rate_limit", lambda: 0.1)
    avl.Factory.set_variable("*.agent.mrdrv.response_rate_limit", lambda: 0.3)
